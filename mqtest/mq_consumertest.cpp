...
kjymq::ConsumerManager::ptr cmp;

class ConsumerTest : public testing::Environment {
    public:
        virtual void SetUp() override {
            cmp = std::make_shared<kjymq::ConsumerManager>();
...
void cb(const std::string &tag, const kjymq::BasicProperties *bp, const std::string &body) {
...
TEST(consumer_test, choose_test) {
    kjymq::Consumer::ptr cp = cmp->choose("queue1");
