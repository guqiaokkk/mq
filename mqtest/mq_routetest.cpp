...
TEST(route_test, legal_routing_key) {
    ...
    ASSERT_EQ(kjymq::Router::isLegalRoutingKey(rkey1), true);
    ASSERT_EQ(kjymq::Router::isLegalRoutingKey(rkey2), true);
    ASSERT_EQ(kjymq::Router::isLegalRoutingKey(rkey3), false);
    ASSERT_EQ(kjymq::Router::isLegalRoutingKey(rkey4), true);
}
TEST(route_test, legal_binding_key) {
    ...
    ASSERT_EQ(kjymq::Router::isLegalBindingKey(bkey1), true);
    ASSERT_EQ(kjymq::Router::isLegalBindingKey(bkey2), true);
    ASSERT_EQ(kjymq::Router::isLegalBindingKey(bkey3), false);
    ASSERT_EQ(kjymq::Router::isLegalBindingKey(bkey4), false);
    ASSERT_EQ(kjymq::Router::isLegalBindingKey(bkey5), false);
    ASSERT_EQ(kjymq::Router::isLegalBindingKey(bkey6), true);
    ASSERT_EQ(kjymq::Router::isLegalBindingKey(bkey7), false);
}
...
TEST(route_test, route) {
    ...
    // aaa.#.bbb.*.bbb      aaa.ddd.ccc.bbb.eee.bbb true
        ...
        for (int i = 0; i < bkeys.size(); i++) {
            ASSERT_EQ(kjymq::Router::route(kjymq::ExchangeType::TOPIC, rkeys[i], bkeys[i]), result[i]);
        }
    }
