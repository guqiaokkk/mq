...
class Server{
    public:
        ...
        typedef std::shared_ptr<kjy::TranslateRequest> TranslateRequestPtr;
        typedef std::shared_ptr<kjy::AddRequest> AddRequestPtr;
        ...
        _codec(std::bind(&ProtobufDispatcher::onProtobufMessage, &_dispatcher, 
            std::placeholders::_1, std::placeholders::_2,std::placeholders::_3)){
                ...
                //注册业务请求处理函数
                _dispatcher.registerMessageCallback<kjy::AddRequest>(std::bind(&Server::onAdd, this,
                    std::placeholders::_1, std::placeholders::_2,std::placeholders::_3));
                
                _dispatcher.registerMessageCallback<kjy::TranslateRequest>(std::bind(&Server::OnTranslate, this,
                    std::placeholders::_1, std::placeholders::_2,std::placeholders::_3));
    ...
    private:
        ...
        void OnTranslate(const muduo::net::TcpConnectionPtr &conn, const TranslateRequestPtr &message, muduo::Timestamp)
        {
            ...
            //2. 进行翻译，得到结果
            std::string ans = translate(req_msg);
             //3. 组织protobuf的响应
            kjy::TranslateResponse resp;
            resp.set_msg(ans);
            //4. 发送响应
        ...
        {
            LOG_INFO << "UnknownMessage: " << message->GetTypeName();
            conn->shutdown();
        }
